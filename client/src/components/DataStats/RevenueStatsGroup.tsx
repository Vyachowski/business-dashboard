  ...
  useEffect(() => {
    const fetchGoalsData = async () => {
      try {
        ...
        const {totalRevenueGoal} = response.data;

        const yearlyResponseRevenue = await axios.get('http://localhost:3011/api/revenue/', {
          params: {
        ...
        const monthlyResponseRevenue = await axios.get('http://localhost:3011/api/revenue/', {
          params: {
        ...
        const weeklyResponseRevenue = await axios.get('http://localhost:3011/api/revenue/', {
          params: {
            startDate: '2022-10-8',
            endDate: '2023-10-15',
            type: 'total',
        ...
        setGoals({
          annualGoal: totalRevenueGoal,
          monthlyGoal: Math.floor(totalRevenueGoal / 12),
          weeklyGoal: Math.floor(totalRevenueGoal / 52),
        })
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-6 2xl:gap-7.5">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="mb-4 text-title-lg font-bold text-black dark:text-white">
                {goals.annualGoal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} RUB
              </h3>
              <p className="font-medium">Annual goal</p>
              <span className="mt-2 flex items-center gap-2">
                ...
                <span className="text-sm font-medium">Since last year</span>
              </span>
            </div>
            ...
            <div className='circle-place'>
              <svg className="h-17.5 w-17.5 -rotate-90 transform">
                ...
                <circle
                  ...
                  strokeDashoffset={
                    30 * 2 * Math.PI - (revenue.annualRevenue / goals.annualGoal) * 30 * 2 * Math.PI
                  }
                  stroke="currentColor"
                  fill="transparent"
                  r="30"
        ...
        <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6 xl:p-7.5">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="mb-4 text-title-lg font-bold text-black dark:text-white">
                {goals.monthlyGoal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} RUB
              </h3>
            ...
            <div x-data="{percent : 30}">
              <svg className="h-17.5 w-17.5 -rotate-90 transform">
                ...
                <circle
                  ...
                  strokeDasharray={30 * 2 * Math.PI}
                  strokeDashoffset={
                    30 * 2 * Math.PI - (revenue.monthlyRevenue / goals.monthlyGoal) * 30 * 2 * Math.PI
                  }
                  stroke="currentColor"
                  fill="transparent"
                  r="30"
        ...
        <div className="rounded-sm border border-stroke bg-white p-4 shadow-default dark:border-strokedark dark:bg-boxdark md:p-6 xl:p-7.5">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="mb-4 text-title-lg font-bold text-black dark:text-white">
                {goals.weeklyGoal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} RUB
              </h3>
            ...
            <div>
              <svg className="h-17.5 w-17.5 -rotate-90 transform">
                ...
                <circle
                  ...
                  strokeDasharray={30 * 2 * Math.PI}
                  strokeDashoffset={
                    30 * 2 * Math.PI - (revenue.weeklyRevenue / goals.weeklyGoal) * 30 * 2 * Math.PI
                  }
                  stroke="currentColor"
                  fill="transparent"
                  r="30"
