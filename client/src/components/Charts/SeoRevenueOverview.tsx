...
const ChartSeven: React.FC = () => {
  const [previousYearSeoRevenue, setPreviousYearSeoRevenue] = useState<number[]>([]);
  const [currentYearSeoRevenue, setCurrentYearSeoRevenue] = useState<number[]>([]);

  const [state, setState] = useState<ChartSevenState>({
    series: [
      {
        name: 'Received Amount',
        data: previousYearSeoRevenue,
      },
      {
        name: 'Due Amount',
        data: currentYearSeoRevenue,
      },
    ],
  });
  ...
  }, [previousYearSeoRevenue, currentYearSeoRevenue]);
  ...
  useEffect(() => {
    ...
    fetchData('2021-09-16', '2022-09-15', setPreviousYearSeoRevenue);
    fetchData('2022-09-16', '2023-09-15', setCurrentYearSeoRevenue);
  }, []);
      ...
      <div className="flex flex-col text-center xsm:flex-row">
        <div className="border-stroke py-2 dark:border-strokedark xsm:w-1/2 xsm:border-r">
          ...
          <h4 className="mt-1 text-title-sm font-bold text-black dark:text-white">
            {currentYearSeoRevenue.reduce((acc, currentValue) => acc + currentValue, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} RUB
          </h4>
        </div>
