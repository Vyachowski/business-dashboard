...
const ChartSeven: React.FC = () => {
  const [previousYearPpcRevenue, setPreviousYearPpcRevenue] = useState<number[]>([]);
  const [currentYearPpcRevenue, setCurrentYearPpcRevenue] = useState<number[]>([]);

  const [state, setState] = useState<ChartSevenState>({
    series: [
      {
        name: 'Received Amount',
        data: previousYearPpcRevenue,
      },
      {
        name: 'Due Amount',
        data: currentYearPpcRevenue,
      },
    ],
  });
  ...
  }, [previousYearPpcRevenue, currentYearPpcRevenue]);
  ...
  useEffect(() => {
    const fetchData = async (startDate, endDate, setRevenueState) => {
      try {
        const response = await axios.get('http://localhost:3011/api/revenue/PPC/', {
          params: {
            startDate,
            endDate,
            type: 'daily',
    ...
    fetchData('2021-09-16', '2022-08-15', setPreviousYearPpcRevenue);
    fetchData('2022-09-16', '2023-08-15', setCurrentYearPpcRevenue);
  }, []);
      ...
      <div className="flex flex-col text-center xsm:flex-row">
        <div className="border-stroke py-2 dark:border-strokedark xsm:w-1/2 xsm:border-r">
          ...
          <h4 className="mt-1 text-title-sm font-bold text-black dark:text-white">
            {currentYearPpcRevenue.reduce((acc, currentValue) => acc + currentValue, 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} RUB
          </h4>
        </div>
